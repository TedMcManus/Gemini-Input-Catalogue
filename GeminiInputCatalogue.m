...
%     s = sqrt(1+dcdx.^2);
    ...
    Qit = (p.Q_amp_h-p.Q_amp_l)*d.*b.*...
         sheet(x3,ctr+p.Q_wth_l/2+p.Q_off_l+p.Q_off_h*p.Q_wth_l/2,p.Q_wth_h*s,p.Q_gsl_h)...
          +p.Q_amp_l*...
         sheet(x3,ctr+p.Q_wth_l/2+p.Q_off_l,p.Q_wth_l*s,p.Q_gsl_l);
    E0it = (p.E_amp_h-p.E_amp_l)*d.*b.*...
         sheet(x3,ctr+p.Q_wth_l/2+p.Q_off_l+p.Q_off_h*p.Q_wth_l/2,p.E_wth_h*s,p.E_gsl_h)...
          +p.E_amp_l*...
         sheet(x3,ctr+p.Q_wth_l/2+p.Q_off_l,p.E_wth_l*s,p.E_gsl_l); % h and l in same pos as Q
    J = J_amp*(...
         sheet(x3,ctr+p.J_wth/2,p.J_wth*s,p.J_gsl)...
        -sheet(x3,ctr-p.J_wth/2,p.J_wth*s,p.J_gsl)...
        );
    F = p.F_amp*(...
         sheet(x3,ctr-p.J_wth,p.F_wth*s,p.F_gsl)...
        -sheet(x3,ctr          ,p.F_wth*s,p.F_gsl)...
        +sheet(x3,ctr+p.J_wth,p.F_wth*s,p.F_gsl)...
        );
    U = F.*(1/sqrt(1+dcdx.^2));
